...
def update():
    server.mario.update()
    for i in range(len(server.platform_s2)):
        if i<2 :
            server.platform_s2[i].y+=0.5
            if server.platform_s2[i].y >= 32 * 15: server.platform_s2[i].y = 0
        else:
            server.platform_s2[i].y -= 0.5
            if server.platform_s2[i].y <= 0: server.platform_s2[i].y = 32 * 15

    collision.player_stage2()
    server.update_s2_item()
    collision.item_player_s2()
    server.enemy_s2_update()
    collision.enemy_player_s2()
